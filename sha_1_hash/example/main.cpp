...
string sha1(const string& strRaw){
    ...
    for (int i(56);i>=0;i-=8){
        str+=(unsigned char)(((unsigned long long)strRaw.size()<<3) >> i);
    }
    vector<unsigned char> test;
    for (int j = 0; j < str.size(); ++j) {
        test.push_back((unsigned char)str[j]);
    }
    vector<unsigned long> re;
    for (int j = 0; j < test.size(); j += 4) {
        unsigned long t = test[j];
        t = (t << 8) + test[j + 1];
        t = (t << 8) + test[j + 2];
        t = (t << 8) + test[j + 3];
        re.push_back(t);
